...
void quicksort(SHOW*, int, int);
void swap (SHOW*,int,int);
bool pesquisaBinaria (SHOW*, char*, int, int);
void createLog(double);
int compareDatesMenor(SHOW, SHOW);
int compareDatesMaior(SHOW, SHOW);
...
void quicksort(SHOW *show, int inicio, int fim) {

    int i = inicio;
    int j = fim;
    SHOW pivo = show[(inicio + fim) / 2];

    while(i <= j){

        while(compareDatesMenor(show[i], pivo) != 0) i++;
        while(compareDatesMaior(show[j], pivo) != 0) j--;
    ...
    logComp++;
    if(inicio < j)
        quicksort(show, inicio, j);

    logComp++;
    if(i < fim)
        quicksort(show, i, fim);

}


int compareDatesMenor(SHOW show, SHOW pivo){
...
int compareDatesMaior(SHOW show, SHOW pivo){
...
void createLog(double tempo){
    ...
    FILE* arq = fopen("866018_selecaoRecursiva.txt", "w");
